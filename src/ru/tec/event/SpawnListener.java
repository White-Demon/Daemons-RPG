 ...
 * @author White2Demon
 *
 *	Áóäåì îáğàáàòûâàòü ñïàâí ìîáîâ ìîäåğíåçèğîâàííûõ
 *
 */
public class SpawnListener implements Listener{
	...
	/**
	 * Ãëàâíîå ñîáûòèå
	 * @param spawn - àïè ñàìî ïîäñòàâèò ıòî
	 */
	@EventHandler
	...
	private void modZombie(Zombie zombie)
	{
		...
		{
			case 0:
				zombie.setCustomName("Çîìáè" + ChatColor.AQUA + " [1 óğîâåíü]");
				zombie.getEquipment().setItemInHand(new ItemStack(Material.STONE_SWORD,1));
				zombie.setVillager(true);
			break;
			case 1:
				zombie.setCustomName("Áåãëûé çîìáè" + ChatColor.GRAY + " [3 óğîâåíü]");
			...
			break;
			case 2:
				zombie.setCustomName("Óìíûé çîìáè" + ChatColor.YELLOW + " [6 óğîâåíü]");
				zombie.getEquipment().setItemInHand(new ItemStack(Material.IRON_SWORD,1));
			...
			break;
			case 3:
				zombie.setCustomName("Ïàğàçîèä" + ChatColor.GOLD + " [9 óğîâåíü]");
			...
			case 4:
				...
				met.setDisplayName(ChatColor.RED + "Ëóê îãíÿ Ëîğäà.");
				bow.setItemMeta(met);
				
				zombie.setCustomName("Ãåğîèäîí" + ChatColor.BLACK + " [ÁÎÑÑ]");
	...
	private void modSkeleton(Skeleton skeleton)
	{
		...
		{
			case 0:
				skeleton.setCustomName("Ñêåëåò " + ChatColor.AQUA + "[1 óğîâåíü]");
			...
			break;
			case 1:
				skeleton.setCustomName("Ñêåëåò ìå÷íèê" + ChatColor.GRAY + "[4 óğîâåíü]");
				skeleton.getEquipment().setHelmet(new ItemStack(Material.IRON_HELMET,1));
			...
			break;
			case 2:
				skeleton.setCustomName("Ñêåëåò îğäåíà " + ChatColor.YELLOW + "[8 óğîâåíü]");
			...
			break;
			case 3:
				skeleton.setCustomName("Ñêåëåò îğäåíà ëó÷íèê " + ChatColor.GOLD + "[12 óğîâåíü]");
				skeleton.getEquipment().setHelmet(new ItemStack(Material.GOLD_HELMET,1));
				skeleton.getEquipment().setBoots(new ItemStack(Material.GOLD_BOOTS,1));
			...
			case 4:
				...
				skeleton.setCustomName("Ëîğä îğäåíà " + ChatColor.BLACK + "[ÁÎÑÑ]");
	...
	private void modEnder(Enderman ender)
	{
		...
		{
			case 0:
				ender.setCustomName("Òåìíûé " + ChatColor.AQUA +" [1 óğîâåíü]");
			...
			break;
			case 1:
				ender.setCustomName("Òåìíûé ÈÎ " + ChatColor.GRAY + "[3 óğîâåíü]");
				ender.getEquipment().setHelmet(new ItemStack(Material.LEATHER_HELMET,1));
				ender.getEquipment().setItemInHand(new ItemStack(Material.IRON_AXE,1));
